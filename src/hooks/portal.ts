...
export const errorMessage1 =
  'You must either add a `ref` to the element you are interacting with or ' +
  'pass an `event` to openPortal(e) or togglePortal(e).'
...
export default function usePortal({
  ...
  const elToMountTo = useMemo(() => (bindTo && findDOMNode(bindTo)) || document.body, [bindTo])
